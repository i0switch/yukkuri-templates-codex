...
字幕の語単位色替え、`dialogue[].emphasis` 由来のSE再生は廃止済み。
...
キャラの驚き、焦り、ツッコミなど、感情リアクションの短い語句だけは、`dialogue[].text` 内で `[[マジで！？]]` のように囲める。
Remotion は `[[...]]` を通常字幕から除去し、発話冒頭の約900msだけ字幕エリア内に大きな単体字幕として表示する。文字色は通常字幕と同じく、喋っているキャラの色に合わせる。
この表示中は通常字幕を隠し、旧 `dialogue[].emphasis`、SE、語単位色替えは使わない。
`[[...]]` は3〜5分動画で0〜2箇所までを目安にし、1発話につき短い1語句だけにする。数字、手順名、結論、行動CTA、説明文全体を囲まない。
