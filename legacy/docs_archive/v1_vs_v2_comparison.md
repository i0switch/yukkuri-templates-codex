...
このファイルは過去の比較記録であり、通常生成の必読入力ではない。
現行の正本は `docs/pipeline_contract.md`、最短入口は `docs/agent_fast_path.md`。
...
| 平均セリフ長 | 短め固定 | 自然な発話単位 | 表示都合の文字数制限を置かず、長い解説は字幕描画側で吸収する |
...
| 既定解像度 | 過去記録 | 現行仕様は FullHD (1920×1080)。ユーザーが明示した場合だけHD/4K等に変更 |
