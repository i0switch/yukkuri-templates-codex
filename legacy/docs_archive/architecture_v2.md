...
この文書は過去の設計思想と背景を説明する補助文書であり、通常生成の必読入力ではない。
現行の初動読込は `docs/agent_fast_path.md` に従う。
...
`script_draft.md` では、自然な会話を優先する。セリフ長は制限しない。表示都合で短く分割せず、長い解説は自然な発話単位で保持する。字幕表示時の折り返しと収まりはRemotion側のBudouX + AutoFitTextで処理し、1シーン6〜12発話を目安にする。
...
- **証跡 (evidence)**: `audits/script_prompt_pack_*.md` 群と `audits/image_prompt_audit.json` / `audits/image_result_audit.json`。prompt pack や各工程の実行記録。`scripts/validate-script-prompt-pack-evidence.mjs` が期待するファイル名で残す。`audits/yaml_conversion_v2.md` は過去項目であり、現行Gateの必須成果物ではない。
