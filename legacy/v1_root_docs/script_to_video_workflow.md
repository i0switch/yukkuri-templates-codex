...
受付後は「入力条件、テンプレート選定、キャラペア、想定尺に矛盾がないか」を確認する。
FAIL の場合は不足情報を補ってから次へ進む。
...
テンプレート読込後は、main/sub/subtitle/title/avoid_area の読み違いを確認する。
不整合があれば修正してから台本生成へ進む。

## 3. 台本生成

台本生成元は `_reference/script_prompt_pack/` のフェーズ別プロンプトを使う。
画像生成元は `_reference/image_prompt_pack/` のフェーズ別プロンプトを使う。
...
- Codexレビュー対象: `script_final.md`
- 修正: `_reference/script_prompt_pack/04_rewrite_prompt.md`
- YAML変換: `_reference/script_prompt_pack/05_yaml_prompt.md`
- 画像正本: `_reference/image_prompt_pack/00_IMAGE_GEN_MASTER_RULES.md`
- 画像方向性: `_reference/image_prompt_pack/01_IMAGE_DIRECTION_PROMPT.md`
- 画像プロンプト: `_reference/image_prompt_pack/02_IMAGEGEN_PROMPT_PROMPT.md`
- 生成前監査: `_reference/image_prompt_pack/03_IMAGE_PROMPT_AUDIT.md`
- 画像計画修正: `_reference/image_prompt_pack/04_IMAGE_REWRITE_PROMPT.md`
- 生成後監査: `_reference/image_prompt_pack/05_IMAGE_RESULT_AUDIT.md`

`00_MASTER_SCRIPT_RULES.md` に含まれる「画面・演出」「BGM」「SE」「テロップ」系の分離ルールに従い、台本段階では素材候補メモとして扱う。
実際の演出指定は後続の `02_演出編集プロンプト.md` で行う。

台本は、動画全体で固定した `meta.layout_template` の表示枠に合わせて作る。
...
- 長い説明は字幕描画側の折り返し、AutoFit、Remotionカードで受ける
  ...
  visual_asset_plan:
    - slot: "main"
      ...
      supports_dialogue:
        - "s01_l01"
        - "s01_l02"
      supports_moment: "視聴者役の勘違いを解説役が止める瞬間"
...
- id: s01
  ...
  visual_asset_plan:
    - slot: main
      ...
      supports_dialogue:
        - "s01_l01"
      supports_moment: "どの掛け合いを補強するか"
      visual_type: "hook_poster"
      composition_type: "smartphone_closeup"
      image_direction:
        dialogue_role: "冒頭フック"
        scene_emotion: "焦り"
        image_should_support: "台本内の補強対象"
        key_visual_sentence: "1枚で伝える状況"
        main_subject: "主役"
        foreground: "前景"
        midground: "中景"
      ...
      imagegen_prompt: |
        ゆっくり解説 / ずんだもん解説動画のScene02 main枠で使う高品質ビジュアル素材。
  dialogue:
    - id: l01
    speaker: left
    text: script_final.md の発話単位を維持したセリフ
```

`asset` は `script/{episode_id}/assets/...` ではなく `assets/...` の相対パスにする。
`visual_asset_plan` はレンダー表示には使わないが、pre-render gate の監査メタとして `script.yaml` に残す。
サブコンテンツ枠がないテンプレートでは `sub: null` にする。
タイトル枠がないテンプレートでは `title_text` に頼らず、必要なら `main.text` へ短い見出しを入れる。

## 5. script_final.mdレビュー

生成後、Codexは `script_final.md` だけをレビュー対象にする。
...
- 字幕表示を理由に台本の発話単位を壊していないか
- 全 `dialogue[].text` が `script_final.md` の自然な発話単位を維持しているか
...
問題がある場合は指摘箇所だけ修正し、`script_final.md` を更新してから素材生成へ進む。
...
- `imagegen_prompt` には「1枚ずつ生成」「この1枚専用」「同一コマンド内で他画像と同時に出力させない（別 Codex プロセスでの並列生成は許可）」相当の文を必ず入れる
- `中央に主題、余白多め`、`licensed photo style`、`clean explainer thumbnail` のような低品質プロンプトで生成しない
- 複数枚必要な場合は、画像ごとに別 Codex プロセスを Bash で並列起動する（同一プロセス内のバッチ生成は禁止、デフォルト並列度 4）

素材生成前に、必ず生成前画像プロンプト監査を実行する。

```powershell
npm run audit:image-prompts -- <episode_id>
```

FAILした場合は、同じプロンプトで押し切らず、`visual_type`、`composition_type`、補強するセリフ、前景/中景/背景から作り直す。
...
`imagegen_prompt` は `image_direction` から展開し、次の完成形式で固定する。
...
`meta.json` には、`file`、`source_site`、`source_type`、`imagegen_prompt`、`imagegen_model`、`scene_id`、`slot`、`purpose`、`adoption_reason`、`license`、`image_direction`、`visual_type`、`supports_dialogue`、`supports_moment` を記録する。
画像の `source_site` または `source_type` は image gen / image_gen / OpenAI image generation 系だと分かる値にする。

生成後は画像を目視監査し、`script/{episode_id}/audits/image_result_audit.json` に保存してから次へ進む。

```powershell
npm run audit:generated-images -- <episode_id>
```

汎用アイコン素材、台本と無関係、字幕やキャラと競合、画像内文字崩れ、感情が動かない画像はFAIL。
FAIL時は同じプロンプトで再生成せず、`visual_type` と構図から作り直す。

素材生成後は、生成素材の採用可否、動画トーンとの一致、meta.json 記録、未解決素材を確認する。
FAIL の場合は再生成または素材計画を修正する。
...
演出加工後は「台本内容を壊していないか」「画像の見せ場が45〜60秒以上空いていないか」「素材が多すぎて読みにくくなっていないか」を確認する。
...
- `dialogue[].text` がすべて `script_final.md` の自然な発話単位を維持している
...
- `node scripts/audit-image-prompts.mjs <episode_id>` が PASS する
- `node scripts/audit-generated-images.mjs <episode_id>` が PASS する
- Codex動画生成の画像素材が image gen 生成として `meta.json` に記録されている
- Codex動画生成の画像素材に `image_direction`、`visual_type`、`supports_dialogue`、`supports_moment` が記録されている
...
`script.yaml` と build/render 前チェックでFAILした場合は、YAML、素材、meta.json の該当箇所を修正する。
...
- `script.yaml` が `meta.layout_template: Scene01`〜`Scene21`、`assets/...`、発話単位を保つ方針に合っている
- 演出加工済み台本がある
