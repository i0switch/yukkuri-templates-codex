...
前提：`10_video-pipeline.md`、`90_asset-license-memo.md`、`_reference/script_prompt_pack/` の現行必須ファイルを既読であること。
...
Claude Code 版 `20_prompt_claude-code.md#2` と同じ品質方針でやる。
ただし新規台本は必ず `_reference/script_prompt_pack/` を使い、Codexレビュー済みの `script_final.md` だけを `script.yaml` に変換する。
...
4. `script_final.md` を作り、Codexレビュー対象にする
5. 必要なら `_reference/script_prompt_pack/04_rewrite_prompt.md` で修正する
6. Codexレビュー済みの `script_final.md` だけを `_reference/script_prompt_pack/05_yaml_prompt.md` で YAML 化する
...
- `script_final.md` のCodexレビュー前に画像生成、音声生成、レンダーへ進む

差分は以下 1 点のみ：

### 素材要件の記述を ImageGen 向けプロンプトに寄せる

各 scene には `visual_asset_plan` を置き、動画内で見やすい素材セットとして設計する。
本編シーンは原則 `main.kind: image` を使い、長尺動画では45〜60秒ごとに最低1つ新しい視覚素材を入れる。
`main.kind: image` / `sub.kind: image` の `asset_requirements` には、`search_keywords` の代わりに `imagegen_prompt` を入れる：

```yaml
visual_asset_plan:
  - slot: "main"
    purpose: "DNAを切る仕組みを一目で理解させる"
    insert_timing: "s02_l01の直前"
    asset: "assets/s02_main.png"
    imagegen_prompt: "ゆっくり解説動画のScene02メイン枠用。..."
    audit_points:
      - "中央主題が1つ"
      - "余白があり字幕やキャラと競合しない"
      - "文字、ロゴ、既存キャラがない"
main:
  ...
  asset_requirements:
    imagegen_prompt: |
      ゆっくり解説動画のScene02メイン枠用。動画全体は明るい青緑系の知的トーン。
      中央にDNAの二重らせん、その一部を分子ハサミ（はさみ型のアイコン）が
      切り取ろうとしている構図。色は青緑系、線はくっきり。
      左右と上下に十分な余白を残し、字幕やキャラと競合しない。
      16:9 アスペクト比。テキスト・文字・数字・ロゴ・既存キャラクター・実在人物は一切入れない。
    style: "フラット・アイコン系"
    aspect: "16:9"
...
7. **動画トーン**を書く（ゆっくり/ずんだもん解説として明るい、少し不穏、知的など）
8. **テンプレート枠**を書く（Scene02メイン枠用、Scene14サブ枠用など）
9. **余白**を書く（字幕、キャラ、背景装飾と競合しない）
