...
成果物名、停止条件、Render schema、画像出所、FullHD既定、voice engine整合、sub枠、motion、review hashは `docs/pipeline_contract.md` を正本にする。このファイルはYAML化作業の手順だけを持つ。
...
- sceneごとに `motion_mode` を付け、会話の山場をレンダーへ渡す。`dialogue[].emphasis` と `**強調語**` は使わない。
- 驚き、焦り、ツッコミなどのリアクション短句だけ、`dialogue[].text` 内で `[[マジで！？]]` のように囲める。3〜5分動画では0〜2箇所まで。数字、手順名、結論、行動CTA、説明文全体を囲まない。
...
- 自然音声尺が allowed_max を超えているだけなのに、尺オーバー対策として `script_final.md` を削除、短文化、要約、圧縮、発話統合しようとしている。
- 最終行動が抽象的。
- 章タイトルが説明目次。
