...
- `imagegen_prompt` には、対象シーンタイトルだけを画像内見出しとして必ず大きく目立たせる指示を入れる。例: `画像内に「月580円が年6,960円に化ける」という見出しを必ず目立つように配置してください。`
- 画像内見出しには `s01` などの scene id を入れない。会話全文も画像内文字として並べない。
...
- `target_duration_sec` は台本密度の目安として残す。5分指定は `270〜330秒` を許容し、短すぎる場合は台本量を増やす。長すぎる場合は自然尺を優先してそのまま使う。
    ...
    visual_asset_plan:
      - slot: main
        ...
        imagegen_prompt: |-
          ...
          画像内に「<対象シーンタイトル>」という見出しを必ず目立つように配置してください。
