...
共通基盤実装       →       シーン 01〜22 順次生成    →    Codex 監査
...
`scene-XX+1` の Step 1 に戻る。シーン 22 まで完了したら Phase 2 へ。

## Phase 2: Codex 監査

44 枚生成完了後、以下を Codex に渡す：
- `07_codex-audit-brief.md` の内容
- `テンプレ/1 (X).jpeg` 22 枚（参照画像）
- `out/scene-XX-rm.png` と `out/scene-XX-zm.png` 計 44 枚
...
```bash
# RM 版 5x5 グリッド（22 枚は 5x5 で 3 枠余る）
ffmpeg -y \
  ...
  -i "out/scene-21-rm.png" -i "out/scene-22-rm.png" \
  -filter_complex "
    ...
    [20:v]scale=384:216[v20];[21:v]scale=384:216[v21];
    color=c=black:s=384x216[pad1];color=c=black:s=384x216[pad2];color=c=black:s=384x216[pad3];
    ...
    [v20][v21][pad1][pad2][pad3]hstack=inputs=5[r5];
...
- Phase 1: 各シーン 10〜15 分 × 22 = 3〜5 時間
