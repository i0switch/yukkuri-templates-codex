# script_final: ep950-rm-free-wifi-trap
...
## Prompt Pack 品質マーカー
- scene_format: 誤解訂正型、失敗エピソード型、あるある型、反証型、Before / After型、手順型、まとめ再フック型を使用。
- viewer_misunderstanding: 鍵マークなら安全、店名っぽいSSIDなら安全、VPNなら万能、自分で押さなければ接続しない、入力後はもう終わり、という誤解を各シーンで扱った。
- reaction_level: L3以上は s01, s03, s04, s05, s07, s09 に配置。
- number_or_example: 4つの重い入力情報、過去接続履歴、4G/5Gへの切替、メール/通販/SNSの使い回し、10分以内の初動など具体例を入れた。
- mini_punchline: 無料の玄関マット、合鍵を配りまくった家、Wi-Fi界のなりすまし店員、スマホのほうが行動力ある、など各シーンに軽いオチを置いた。
- セルフ監査: 10シーン、100セリフ、魔理沙3連続なし、Scene01 sub null、最後は自動接続確認という具体行動。
