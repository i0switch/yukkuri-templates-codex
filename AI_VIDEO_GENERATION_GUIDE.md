...
- `bgm:` ブロックを必須で記載
- `dialogue[].text` 内の `**強調語**` と `dialogue[].emphasis` は使わない。旧 episode に残っていてもrendererは無視する
- 驚き、焦り、ツッコミなどの感情リアクションを一瞬だけ強く出したい場合は、短い語句だけ `[[マジで！？]]` のように囲める。Remotion側で通常字幕からマーカーを除去し、発話冒頭に字幕エリア内の単体バウンド字幕として表示する。文字色は喋っているキャラの字幕色に合わせる
- `[[...]]` は3〜5分動画で0〜2箇所までを目安にし、1発話につき短い1語句だけにする。数字、手順名、結論、行動CTA、説明文全体を囲まない。表示中は通常字幕を隠す
