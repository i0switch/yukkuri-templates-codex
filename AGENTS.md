...
- `recommended_action: keep_natural_overrun_do_not_trim`、`duration_status: over`、allowed_max超過warningが出た場合、尺合わせ目的の削除、短縮、要約、圧縮、発話統合は禁止する
- オーバー時に許される修正は、事実誤り、重複、キャラ崩れ、文脈破綻など品質理由の局所修正だけ。修正後は必ず `script_final_review.md` を再作成し、hash一致を確認する
...
台本の良し悪しはLLMが本文を読んで判断する。機械ゲートは文字数、発話数、平均文字数、キーワード充足で台本品質を合否判定しない。
レビューでは、フック、掛け合い、具体例、視聴維持、キャラ口調、説明bot化、行動CTA、尺オーバー由来の不自然な削除有無を見て `PASS` / `FAIL` を出す。
`FAIL` の場合は、どこが悪いか、どこをどう直すかを書く。
